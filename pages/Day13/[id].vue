...
const { roomDetail, getRoomDetail } = useHotel();
...
const ogUrl = computed(() => `https://freyja.travel.com.tw/room/${roomDetail.value._id}`);
</script>
...
<template>
    <Head>
        ...
        <Meta property="og:description" :content="roomDetail.description"/>
        <Meta property="og:image" :content="roomDetail.imageUrl"/>
        <Meta property="og:url" :content="ogUrl"/>
        <Meta name="twitter:card" content="summary_large_image"/>
        <Meta name="twitter:title" :content="title"/>
        <Meta name="twitter:description" :content="roomDetail.description"/>
        <Meta name="twitter:image" :content="roomDetail.imageUrl"/>
    </Head>
    <h2>房型詳細頁面</h2>

    <div class="container">
        <button @click="router.push('/Day13')" class="btn btn-primary">回上一頁</button>
        <ClientOnly>
                ...
                <div class="col-md-6">
                    <div class="room-page">
                        ...
                        <div class="room-gallery">
                            <img :src=roomDetail.imageUrl :alt=roomDetail.name class="room-main-image" />
                            <div class="room-image-list">
                                <img v-for="(image, index) in roomDetail.imageUrlList" :key="index" :src="image">
                            </div>
                        </div>
